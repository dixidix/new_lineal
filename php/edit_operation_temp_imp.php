...
if (empty($errors)){
	...
	if(!empty($_FILES['file_ctit'])){
		MysqliDB::getInstance()->query("UPDATE `temp_impo` SET `ctit`='".$ctit."',`start_date`='".$start_date."',`request_exp`='".$request_exp."',`filename`='".$file_name_pdf."',`fileSystemname`='".$fileSystemname_pdf."',`owner`='".$owner."' WHERE temp_impoId='".$temp_impoId."'");
	}else{
		MysqliDB::getInstance()->query("UPDATE `temp_impo` SET `ctit`='".$ctit."',`start_date`='".$start_date."',`request_exp`='".$request_exp."',`owner`='".$owner."' WHERE temp_impoId='".$temp_impoId."'");
	}
}
?>
